...
const Compaines = ({ curretnCompany, type, resource, certification }) => {
  ...
  const submitFormHandler = (e) => {
    ...
    const filteredCompanies = companies.filter(
      (comp) => comp.id !== selectedComapny.id
    );
  ...
  return (
    <>
      ...
      <button className="AddNewComp" onClick={addNewCompanyHandler}>
        {type === "about_page" ? "+ إضافة خبرة جديدة" : `+ إضافة ${x}`}
      </button>
    </>
  );
};
